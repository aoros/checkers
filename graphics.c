...
void ResetBoard(void) {
    ...
    for (y = 0; y < 8; y++)
        for (x = 0; x < 8; x++) {
            if (x % 2 != y % 2) {
                square[y][x].val = pos;
                if (y < 3 || y > 4) square[y][x].state = Piece;
                else square[y][x].state = Empty;
                if (y < 3) square[y][x].col = Red;
                else
                    if (y > 4) square[y][x].col = White;
                else
...
int NewDialog(char *player1, char *player2, float *SecPerMove) {
    ...
    strcpy(player1, field1);
    strcpy(xval[0], field1);
    XtFree(field1);
    strcpy(player2, field2);
    strcpy(xval[1], field2);
    XtFree(field2);
    *SecPerMove = (float) atof(field3);
    strcpy(xval[2], field3);
    XtFree(field3);
...
Widget CreateWorkWin(Widget main) {
    ...
    if (width < 640 || height < 640) squaresize = 40;
    else
        if (width < 800 || height < 800) squaresize = 50;
    else
        if (width < 960 || height < 960) squaresize = 60;
    else
        if (width < 1120 || height < 1120) squaresize = 70;
    else
        squaresize = 80;
