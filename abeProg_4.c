...
int LowOnTime(void) {
    ...
    current = times(&bff);
    total = (float) ((float) current - (float) start) / CLK_TCK;
    if (total >= (SecPerMove - 1.0)) return 1;
    else return 0;
}
...
void FindBestMove(int player) {
    ...
    for (x = 0; x < state.numLegalMoves; x++) {
        ...
        double val;
        memcpy(newState.board, state.board, 64 * sizeof (char));
...
// perform move
// score = maxval on resulting board
// if minscore> score then minscore= score
// return minscore

// MaxVal
// gen legal moves
// for every legal move in legalMoves
// perform the move
// score = call minval on resulting board
// if maxscore < score, then maxscore = score
// return maxscore
...
void PerformMove(char board[8][8], char move[12], int mlen) {
    ...
    if (abs(x2 - x) == 2) {
        for (i = 0, j = 1; j < mlen; i++, j++) {
            if (move[i] > move[j]) {
                y1 = -1;
                if ((move[i] - move[j]) == 9) x1 = -1;
                else x1 = 1;
            } else {
                y1 = 1;
                if ((move[j] - move[i]) == 7) x1 = -1;
                else x1 = 1;
            }
...
/**
 * Using www.cs.huji.ac.il/~ai/projects/old/English-Draughts.pdf
 * @param currBoard
 * @param x
 * @param y
 * @param color
...
int pieceAndRowToValue(State *currBoard, int x, int y, int color, int numPiecesOnBoard) {
    int kingFactor = 8;
    int pawnFactor = 5;
    int endGameBaseFactor = 5;
    int endGameStartCount = 6;
...
int spotHasPiece(State *currBoard, int x, int y) {
    if (king(currBoard->board[x][y]) || piece(currBoard->board[x][y]))
        return 1;
    else
        return 0;
}
